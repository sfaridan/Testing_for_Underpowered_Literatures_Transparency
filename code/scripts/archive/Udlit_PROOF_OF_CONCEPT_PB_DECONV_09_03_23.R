...
theta <- 0.5
tc <- 1.96
data_pre <- rnorm(5000,mean=2,sd=2)
...
lines(xs, dnorm((xs-2)/sqrt(5))/sqrt(5),col="red")
cvd <- deconv_approx_fun(xs)
plot(xs,cvd,main="Deconvolution with pb removed")
lines(xs, dnorm((xs-2)/sqrt(3))/sqrt(3),col="red")
